...
class Index extends Backend
{
    ...
    protected $noNeedLogin = ['login','oauth'];
    ...
    public function oauth()
    {
        ...
        if(isset($username)&&$username&&isset($timestamp)&&$timestamp&&isset($key)&&$key){
            //当前时间戳 -  传过来的时间戳
            $diff = time() - $timestamp;
            //在5分钟内
            if($diff < 5*60){
                $new_key = md5($username.$timestamp."123!@#");
                $keeplogin = 0;
                if($key == $new_key){
                    $result = $this->auth->oauthLogin($username, $keeplogin ? 86400 : 0);
                    if ($result === true) {
                    ...
                    $this->error("登录秘钥错误", $url);
                }
            }else{
                $this->error("登录秘钥已经超过时间", $url);
            }
        }else{
            $this->error("参数错误", $url);
        }
